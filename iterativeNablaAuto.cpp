...
real L( const ArrayXreal& x ) // returns L2 norm squared, but in terms of the Eigen library
{
    real L = 0.0;
    for( int i = 0; i < x.size(); i+=2 )
    {
        L += x( i ) * x( i ) + x( i+1 ) * x( i+1 ); // removed 2s, probably correct now
    }
    return L;
}

const int MAX_ITERATIONS = 1e3 + 1;
